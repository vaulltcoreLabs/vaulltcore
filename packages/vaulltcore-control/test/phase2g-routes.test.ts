...
describe("protected /identity/* pipeline", () => {
  ...
  it("membership administration: add → role change → remove; viewer is forbidden; invalid role is 422; cross-org is 404", async () => {
    ...
    // B6: session stays technically valid from the revocation ledger's perspective —
    // the memberless actor now has zero org context (orgId ""), so every org-scoped
    // authorization below (including this listing) returns 404 immediately..
    expect(memberAfter.status).toBe(200)
    expect(memberAfter.body.orgId).toBe("")
    expect(memberAfter.body.role).toBe("viewer")
    const orphanScoped = await json(rig.base, "GET", "/identity/orgs/org-acme/members", { cookie: member.cookie })
    expect(orphanScoped.status).toBe(404)
  })
...
describe("organization invitations (B6/B7)", () => {
  it("owner-only issuance; token returned once; list never leaks it; viewer is 403; cross-org is 404", async () => {
    const rig = await serve()
    const owner = await signUp(rig, "inv-owner@example.com")
    await joinAsOwner(rig, owner.userId)
    const viewer = await signUp(rig, "inv-viewer@example.com")
    await rig.identity.registerPrincipal(rig.tenantId, viewer.userId, "user").catch(() => undefined)
    await rig.identity.addMember(rig.tenantId, rig.orgId, viewer.userId, "viewer")
    const create = await json(rig.base, "POST", "/identity/orgs/org-acme/invitations", { cookie: owner.cookie, body: { email: "newbie@example.com", role: "developer" } })
    expect(create.status).toBe(201)
    expect(create.body.token).toBeTruthy()
    expect(create.body.invitation.status).toBe("pending")
    const list = await json(rig.base, "GET", "/identity/orgs/org-acme/invitations", { cookie: owner.cookie })
    expect(list.status).toBe(200)
    expect(list.body.invitations[0].token).toBeUndefined()
    expect(JSON.stringify(list.body)).not.toContain(create.body.token)
    const dup = await json(rig.base, "POST", "/identity/orgs/org-acme/invitations", { cookie: owner.cookie, body: { email: "newbie@example.com", role: "developer" } })
    expect(dup.status).toBe(409)
    const forbidden = await json(rig.base, "POST", "/identity/orgs/org-acme/invitations", { cookie: viewer.cookie, body: { email: "x@example.com", role: "viewer" } })
    expect(forbidden.status).toBe(403)
    const cross = await json(rig.base, "POST", "/identity/orgs/org-nope/invitations", { cookie: owner.cookie, body: { email: "x@example.com", role: "viewer" } })
    expect(cross.status).toBe(404)
  })

  it("one-time accept via authenticated session; role frozen; email-ownership enforced; replay is 404", async () => {
    const rig = await serve()
    const owner = await signUp(rig, "inv2-owner@example.com")
    await joinAsOwner(rig, owner.userId)
    const invitee = await signUp(rig, "invitee@example.com")
    const created = await json(rig.base, "POST", "/identity/orgs/org-acme/invitations", { cookie: owner.cookie, body: { email: "invitee@example.com", role: "developer" } })
    expect(created.status).toBe(201)
    const accept = await json(rig.base, "POST", "/identity/invitations/accept", { cookie: invitee.cookie, body: { token: created.body.token } })
    expect(accept.status).toBe(201)
    expect(accept.body.role).toBe("developer")
    expect(accept.body.orgId).toBe("org-acme")
    const me = await json(rig.base, "GET", "/identity/me", { cookie: invitee.cookie })
    expect(me.status).toBe(200)
    expect(me.body.role).toBe("developer")
    expect(me.body.orgId).toBe("org-acme")
    const replay = await json(rig.base, "POST", "/identity/invitations/accept", { cookie: invitee.cookie, body: { token: created.body.token } })
    expect(replay.status).toBe(404)
    // A DIFFERENT authenticated user whose Better Auth email differs cannot ride the invite..
    const other = await signUp(rig, "other-invitee@example.com")
    const created2 = await json(rig.base, "POST", "/identity/orgs/org-acme/invitations", { cookie: owner.cookie, body: { email: "invitee@example.com", role: "developer" } })
    const badAccept = await json(rig.base, "POST", "/identity/invitations/accept", { cookie: other.cookie, body: { token: created2.body.token } })
    expect(badAccept.status).toBe(403)
  })

  it("expired or revoked invitations never accept;resend kills the old token", async () => {
    const rig = await serve()
    const owner = await signUp(rig, "inv3-owner@example.com")
    await joinAsOwner(rig, owner.userId)
    const invitee = await signUp(rig, "invitee3@example.com")
    const expired = await json(rig.base, "POST", "/identity/orgs/org-acme/invitations", { cookie: owner.cookie, body: { email: "invitee3@example.com", role: "viewer", expiresInMs: -1000 } })
    expect(expired.status).toBe(201)
    const expiredAccept = await json(rig.base, "POST", "/identity/invitations/accept", { cookie: invitee.cookie, body: { token: expired.body.token } })
    expect(expiredAccept.status).toBe(409)
    const revoked = await json(rig.base, "POST", "/identity/orgs/org-acme/invitations", { cookie: owner.cookie, body: { email: "revoked3@example.com", role: "viewer" } })
    expect(revoked.status).toBe(201)
    const revoke = await json(rig.base, "POST", "/identity/orgs/org-acme/invitations/" + revoked.body.invitation.invitationId + "/revoke", { cookie: owner.cookie })
    expect(revoke.status).toBe(200)
    const revokedAccept = await json(rig.base, "POST", "/identity/invitations/accept", { cookie: invitee.cookie, body: { token: revoked.body.token } })
    expect(revokedAccept.status).toBe(404)
    const resend = await json(rig.base, "POST", "/identity/orgs/org-acme/invitations", { cookie: owner.cookie, body: { email: "invitee3@example.com", role: "viewer" } })
    expect(resend.status).toBe(201)
    const resent = await json(rig.base, "POST", "/identity/orgs/org-acme/invitations/" + resend.body.invitation.invitationId + "/resend", { cookie: owner.cookie })
    expect(resent.status).toBe(201)
    expect(resent.body.token).not.toBe(resend.body.token)
    const oldAccept = await json(rig.base, "POST", "/identity/invitations/accept", { cookie: invitee.cookie, body: { token: resend.body.token } })
    expect(oldAccept.status).toBe(404)
    const newAccept = await json(rig.base, "POST", "/identity/invitations/accept", { cookie: invitee.cookie, body: { token: resent.body.token } })
